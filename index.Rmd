...
df %<>%
  ...
  mutate(
    ...
    dailyEstimatedActualCases = 100*dailyPositiveTests^2/dailyTests,
    dailyEstimatedActualCasesRollMean = rollmean(dailyEstimatedActualCases,7, na.pad=TRUE, align="right")) %>%
  as_tibble()
...
```{r estimatedCases, echo=FALSE}
fig <- plot_ly(df_US, x = ~date, y = ~dailyEstimatedActualCases, type = 'scatter', mode = 'lines', color = I('gray'), name = 'Daily', width=1000)
fig <- fig %>% add_trace(y = ~dailyEstimatedActualCasesRollMean, color = I('red'), name = '7 day rolling average')
fig <- fig %>% layout(legend = list(orientation = 'v'))
fig <- fig %>% layout(yaxis = list(title = "Estimated Actual Cases [thousands]"))
